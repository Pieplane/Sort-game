...
public class CollectionPopupUI : MonoBehaviour
{
    ...
    public IEnumerator PlayCompleteEffectDOTween()
    {
        ...
        if (progressText != null)
            progressText.text = GetItemCollectedText(); // ✅ локализация
    ...
    string GetItemCollectedText()
    {
        string lang = GetLang();

        if (lang.Contains("ru")) return "Предмет собран!";
        if (lang.Contains("en")) return "Item collected!";
        if (lang.Contains("fr")) return "Objet collecté !";
        if (lang.Contains("de")) return "Gegenstand gesammelt!";
