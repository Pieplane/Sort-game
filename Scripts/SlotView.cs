...
public class SlotView : MonoBehaviour, IDropHandler
{
    ...
    public ItemView GetItemView()
    {
        ...
        for (int i = 0; i < contentRoot.childCount; i++)
        {
            ...
            // ✅ считаем "настоящим" предметом ТОЛЬКО то, что можно таскать
            var drag = child.GetComponent<DraggableItem>();
            if (drag == null) continue;

            var iv = child.GetComponent<ItemView>();
    ...
    public void OnDrop(PointerEventData eventData)
    {
        ...
        if (HasItem)
        {
            if (shelf != null)
            {
                var empty = shelf.FindFirstEmptySlot();
                if (empty != null)
                {
                    empty.AcceptItem(item);
                    board.OnItemPlaced(empty);
                    return;
                }
            }

            // пустых нет — вернуть назад
            item.ReturnToOrigin();
            return;
        }
        AcceptItem(item);
    ...
    public void AcceptItem(DraggableItem item)
    {
        ...
        //bool willBeTriple = (shelf != null && shelf.IsTripleNow());
        //if (!willBeTriple)
        //{
