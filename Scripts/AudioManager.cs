...
public class AudioManager : MonoBehaviour
{
    ...
    [SerializeField] private float collectCooldown = 0.08f; // 0.06–0.12
    ...
    //public void PlayCollectOncePerFrame()
    //{
    //    if (Time.frameCount == lastCollectFrame) return;
    //    lastCollectFrame = Time.frameCount;
    //    Play("Collect");
    //}
    public void PlayCollectCooled()
    {
        if (Time.unscaledTime < nextCollectTime) return;
        nextCollectTime = Time.unscaledTime + collectCooldown;
        Play("Collect");
    }
}
