...
public class XpFlyDOTween : MonoBehaviour
{
    ...
    [Header("SFX")]
    [SerializeField] private float sfxCooldown = 0.2f; // 0.06–0.12 норм
    ...
    void SpawnOne(Vector2 startPos, float delay, bool isLast)
    {
        ...
        // полёт (сохраняем tween в переменную, чтобы повесить звук на прилёт)
        Tween moveTween = rt.DOAnchorPos(targetPos, flyTime).SetEase(Ease.InQuad);
        seq.Append(moveTween);
        seq.Join(rt.DOScale(0.2f, flyTime));

        // ✅ звук на прилёт, но не слишком часто
        moveTween.OnComplete(() =>
        {
            if (AudioManager.Instance != null && Time.unscaledTime >= nextSfxTime)
            {
                nextSfxTime = Time.unscaledTime + sfxCooldown;
                AudioManager.Instance.Play("Score"); // <-- имя твоего XP SFX в AudioManager
            }
        });
