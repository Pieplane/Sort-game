...
public class RoomUpgradePanel : MonoBehaviour
{
    ...
    public void Open(RoomItemDefinition def)
    {
        ...
        if (PlayerProgress.Instance != null && !PlayerProgress.Instance.IsLoaded)
            PlayerProgress.Instance.OnLoaded += RefreshAfterLoad;
    ...
    void Refresh()
    {
        ...
        for (int i = 0; i < max; i++)
        {
            ...
            if (card.titleText != null)
            {
                card.titleText.text = string.IsNullOrEmpty(v.title)
                ? $"{GetVariantWord()} {idx}"
                : v.title;
            }
            ...
            if (card.lockText != null)
                card.lockText.text = (!unlocked) ? $"{GetRankWord()} {v.unlockLevel}" : "";
            ...
            if (!unlocked)
            {
                ...
                if (card.btnUnlockAd)
                {
                    card.btnUnlockAd.onClick.AddListener(() =>
                    {
                        if (AdController.Instance == null)
                        {
                            Debug.LogWarning("AdController not found");
                            return;
                        }
                        ...
                        AdController.Instance.ShowRewardAdv(
                            onRewardFinished: () =>
                            {
                                UnlockAdAndEquip(idx); // ✅ награда ТОЛЬКО после рекламы
                            },
                            rewardType: RewardType.UnlockAndEquipItem,
                            onClosedWithoutReward: () =>
                            {
                                // закрыли рекламу — ничего не делаем
                            }
                        );
                    });
                }
    ...
    string GetLang()
    {
        return (Language.Instance != null && !string.IsNullOrEmpty(Language.Instance.CurrentLanguage))
            ? Language.Instance.CurrentLanguage.ToLowerInvariant()
            : "en";
    }
    string GetVariantWord()
    {
        string lang = GetLang();

        if (lang.Contains("ru")) return "Вариант";
        if (lang.Contains("en")) return "Variant";
        if (lang.Contains("fr")) return "Variante";
        if (lang.Contains("de")) return "Variante";
